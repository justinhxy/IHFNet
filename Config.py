...
def parse_args():
    ...
    return args
"""
你现在很懂overleaf的写作，请基于以下的文字进行修改：
\textbf{Implementation Details.} All experiments were carried out using PyTorch 2.6.0 on a single Nvidia V100 32G GPU and employing 5-fold cross-validation. The model was trained from scratch over two distinct stages, each consisting of 150 epochs, with a batch size of 8 to effectively manage the data. We optimized the model parameters using the Adam algorithm, setting the learning rate to 0.001 to ensure precise adjustments during training.
注意：我们使用的是【pytorch 2.6.0 和cuda 版本 11.8】，训练轮数是150轮，学习率为0.001，使用CosineLRScheduler 调度器，其中
超参数【T_max为50】
"""